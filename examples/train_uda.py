...
def get_train_loader(args, dataset, height, width, batch_size, workers,
                    num_instances, iters, trainset=None, cam_flag=0):
    ...
    if cam_flag:
        train_loader = IterLoader(
            DataLoader(
                UnsupervisedCamStylePreprocessor(train_set, root=dataset.images_dir, camstyle_root=dataset.camstyle_dir,
...
def main_worker(args):
    ...
    memory = HybridMemory(model.module.num_features, source_classes+len(dataset_target.train), temp=args.temp,
                          momentum=args.momentum, num_source=dataset_source.num_train_pids).cuda()
    ...
    memory.target_cams = torch.tensor([cid for _, _, cid in sorted(dataset_target.train)])  #
    ...
    del sour_cluster_loader, sour_fea_dict
    ...
    for epoch in range(args.epochs):
        ...
        target_features, _ = extract_features(model, tgt_cluster_loader, print_freq=50)
        cluster_features = torch.cat([target_features[f].unsqueeze(0) for f, _, _ in sorted(dataset_target.train)], 0)
        rerank_dist = compute_jaccard_distance(cluster_features, k1=args.k1, k2=args.k2)
        ...
        for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset_target.train), pseudo_labels)):
            ...
            if ((indep_score<=indep_thres) and (comp_score.item()<=cluster_R_comp[label.item()-source_classes])):
                ...
                #cope_label.append(label.item())
                #after_label.append(source_classes+len(cluster_R_indep)+outliers)
        ...
        """
        cope_label = sorted(set(cope_label))
        cope_len = len(cope_label)
        print(cope_label)
        #after_label = sorted(set(after_label))
        #print(after_label)
        for i, (f, label, c) in enumerate(pseudo_labeled_dataset):
              if label < cope_label[0]:
                  continue
              if label > cope_label[cope_len-1]:
                  pseudo_labeled_dataset[i] = (f, label - cope_len, c)
                  pseudo_labels[i] = label - cope_len
                  continue
              for j in range(cope_len-1):
                  if label > cope_label[j] and label < cope_label[j+1]:
                      pseudo_labeled_dataset[i] = (f, label-j-1, c)
                      pseudo_labels[i] = label-j-1
                      break
        print(pseudo_labels)   #
        """
        ...
        for f, pid, _ in sorted(pseudo_labeled_dataset):
        ...
        all_features = torch.cat((source_centers, F.normalize(target_centers, dim=1)), dim=0).cuda()
        del target_centers
        ...
        label_sets = torch.tensor(list(index2label.keys())+[i for i in range(source_classes)]).unique()   #
        memory.features = torch.zeros(label_sets.max()+1, all_features.size(1)).cuda()
        memory.features[label_sets] = all_features
        memory.label_sets = label_sets.cuda()
        ...
        train_loader_target = get_train_loader(args, dataset_target, args.height, args.width,
                                            args.batch_size, args.workers, args.num_instances, iters,
                                            trainset=pseudo_labeled_dataset, cam_flag=0)   #
