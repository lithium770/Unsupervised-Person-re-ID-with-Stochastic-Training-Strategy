...
from reid.trainers import SpCLTrainer_USL
...
def get_train_loader(args, dataset, height, width, batch_size, workers,
                    num_instances, iters, trainset=None, cam_tag=0):
    ...
    if cam_tag:
        train_loader = IterLoader(
            DataLoader(UnsupervisedCamStylePreprocessor(train_set, root=dataset.images_dir, camstyle_root=dataset.camstyle_dir,
                       num_cam=dataset.num_train_cams, transform=train_transformer),
...
def main_worker(args):
    ...
    memory.instance_features = F.normalize(features, dim=1).cuda()
    ...
    cams = dataset.num_train_cams

    history_weight = torch.zeros(dataset.num_train_imgs).cuda()

    for epoch in range(args.epochs):
        cluster_loader = get_test_loader(dataset, args.height, args.width,  #
                                         args.batch_size, args.workers, testset=sorted(dataset.train))  #
        target_features, _ = extract_features(model, cluster_loader, print_freq=50)  #

        if epoch > 0:
            features = torch.cat([target_features[f].unsqueeze(0) for f, _, _ in sorted(dataset.train)], 0)
            memory.instance_features = F.normalize(memory.instance_features*history_weight + (features.cuda())*(1-history_weight), dim=1)    # update features for outliers

        # Calculate distance
        print('==> Create pseudo labels for unlabeled data with self-paced policy')
        features = memory.instance_features.clone().cuda()
        #features = torch.cat([target_features[f].unsqueeze(0) for f, _, _ in sorted(dataset.train)], 0)
        #rerank_dist = compute_jaccard_distance(features, k1=args.k1, k2=args.k2)
        rerank_dist = re_ranking(features, cam_labels, cams, k1=args.k1, k2=args.k2)
        del features
        ...
        cluster_labels = []
        inds = []    # store the global index for cluster instances
        weight = []
        #outliers = 0
        for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset.train), pseudo_labels)):
            ...
            #comp_score = R_comp[i]
            #if label.item() < num_ids and ((indep_score<=indep_thres) and (comp_score.item()<=cluster_R_comp[label.item()])):
            if label.item() < num_ids:
                pseudo_labeled_dataset.append((fname,label.item(),cid))
            else:
                #pseudo_labeled_dataset.append((fname,len(cluster_R_indep)+outliers,cid))
                pseudo_labeled_dataset.append((fname, num_ids, cid))
                #pseudo_labels[i] = len(cluster_R_indep)+outliers
                pseudo_labels[i] = num_ids
                outliers+=1
            """
            if label.item() < num_ids:
                pseudo_labeled_dataset.append((fname, label.item(), cid))
        ...
        history_weight = torch.tensor(weight).unsqueeze(1).float().cuda()   #
        memory.global_ind = torch.tensor(inds).cuda()

        # Initialize cluster memory
        tar_fea_dict = collections.defaultdict(list)
        for i, (f, pid, _) in enumerate(sorted(pseudo_labeled_dataset)):
            #tar_fea_dict[pid].append(target_features[f].unsqueeze(0))
            tar_fea_dict[pid].append(memory.instance_features[inds[i]].unsqueeze(0))    # using features in instance memory
        #target_centers = [torch.cat(tar_fea_dict[pid], 0).mean(0) for pid in sorted(tar_fea_dict.keys())]
        #target_centers = torch.stack(target_centers, 0)

        target_centers = []
        for pid in sorted(tar_fea_dict.keys()):
            # random sample an instance to initialize each clusters
            key_features = torch.cat(tar_fea_dict[pid], 0)
            target_centers.append(key_features[int(torch.round(torch.rand(1) * (key_features.size(0)-1)))].unsqueeze(0))
        target_centers = torch.cat(target_centers, 0)
        ...
        memory.cluster_features = target_centers.cuda()   #
        ...
        memory.labels = torch.tensor(cluster_labels).cuda()
        train_loader = get_train_loader(args, dataset, args.height, args.width,
                                            args.batch_size, args.workers, args.num_instances, iters,
                                            trainset=pseudo_labeled_dataset, cam_tag=0)

        train_loader.new_epoch()

        trainer.train(epoch, train_loader, optimizer,
                    print_freq=args.print_freq, train_iters=len(train_loader), outlier_label=num_ids)
