...
class SpCLTrainer_UDA(object):
    ...
    def train(self, epoch, data_loader_source, data_loader_target,
        ...
        for i in range(train_iters):
            ...
            #t_inputs, t_trans, _, t_indexes, t_cids = self._parse_dataT(target_inputs)
            ...
            #s_inputs, t_inputs, t_trans = reshape(s_inputs), reshape(t_inputs), reshape(t_trans)
            #inputs = torch.cat((s_inputs, t_inputs, t_trans), 1).view(-1, C, H, W)
            ...
            #f_out_s, f_out_t, f_out_trans = f_out.split(f_out.size(1)//3, dim=1)
            #f_out_s, f_out_t, f_out_trans = f_out_s.contiguous().view(-1, f_out.size(-1)), f_out_t.contiguous().view(-1, f_out.size(-1)), \
            #                                f_out_trans.contiguous().view(-1, f_out.size(-1))
            ...
            #loss_t = self.memory(f_out_t, t_indexes+self.source_classes, knn=4, cids=t_cids, trans=f_out_trans)
            loss_t = self.memory(f_out_t, t_indexes+self.source_classes, is_target=True)
...
class SpCLTrainer_USL(object):
    ...
    def train(self, epoch, data_loader, optimizer, print_freq=10, train_iters=400):
        ...
        for i in range(train_iters):
            ...
            # process inputs
            #inputs, trans, _, indexes = self._parse_data(inputs)
            inputs, _, indexes = self._parse_data(inputs)

            # forward
            f_out = self._forward(inputs)
            #trans = self._forward(trans)

            # compute loss with the hybrid memory
            loss = self.memory(f_out, indexes, is_target=True)
            #loss = self.memory(f_out, indexes, trans=trans)
            ...
            #tlosses.update(trans_loss.item())
    ...
    def _parse_data(self, inputs):
        #imgs, tran_imgs, _, pids, _, indexes = inputs
        #return imgs.cuda(), tran_imgs.cuda(), pids.cuda(), indexes.cuda()
        imgs, _, pids, _, indexes = inputs
        return imgs.cuda(), pids.cuda(), indexes.cuda()
