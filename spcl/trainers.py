...
class SpCLTrainer_UDA(object):
    ...
    def train(self, epoch, data_loader_source, data_loader_target,
        ...
        for i in range(train_iters):
            ...
            # process inputs
            s_inputs, s_targets, _, _ = self._parse_data(source_inputs)
            t_inputs, _, t_indexes, t_cids = self._parse_data(target_inputs)
            ...
            loss_s = self.memory(f_out_s, s_targets, knn=-1)
            loss_t = self.memory(f_out_t, t_indexes + self.source_classes, knn=4, cids=t_cids)
...
class SpCLTrainer_USL(object):
    ...
    def train(self, epoch, data_loader, optimizer, print_freq=10, train_iters=400, outlier_label=0):
        self.encoder.train()
